...
//------------------------------VARIABLES-------------------------------------\\    
    ...
    private int amountOfSoldiers = 5;
    ...
    private double uniqueBonusSoldier = 1;
...
//---------------------------------GUI----------------------------------------\\    
    @FXML
    private Label goldLabel, woodLabel, stoneLabel, foodLabel, humanLabel, ironLabel, coalLabel, steelLabel, currentTurnLabel, activeUserLabel, statsViewBuildingLabel, statsViewNoteLabel, scoreLabel, cheatCodeLabel, cheatCodeErrorLabel, soldierLabel;
    ...
    private Button houseButton, woodmillButton, farmButton, stonemasonryButton, bankButton, marketButton, ironMineButton, coalMineButton, storageButton, steelworksButton, cottageButton, nextTurnButton,
            graneryButton, sawmillButton, bazaarButton, stoneworksButton, lumberjackButton, schoolButton, aqueductButton, workshopButton, ingameMenuButton, resumeGameButton, cheatCodeButton, backToMainMenuButton, exitGame, cheatCodesOkButton, barrackButton;
...
//------------------------------FXML METHODS----------------------------------\\
    ...
    private void showStatsViewColumns(MouseEvent event) {
        ...
        switch (statsViewBuildingName) {
            ...
            case "Barrack":
                refreshStatsViewColumns(true, 5);
                statsViewNoteLabel.setText("Note: Produce soldiers.");
                break;
        }
...
//----------------------------NON-FXML METHODS--------------------------------\\
    ...
    private void refreshResources(){
        ...
        soldierLabel.setText("Soldier: " + amountOfSoldiers + "/" + resourceCap);
    }
    ...
    private void addResourcesToAmount(){
        ...
        amountOfSoldiers += resourceList.get(7).getByTurn().getValue();
        resourceCapHandler();
    }
    ...
    private void resourcePerTurnCalc(){
        ...
        for(int i = 0; i < normalBuildingList.size(); i++){
            ...
            resourceList.get(7).setByTurn((int) (normalBuildingList.get(i).getAmount().getValue() * normalBuildingList.get(i).getSoldier() * EventStorage.getInstance().getEventChangeSoldierMultiplier() * uniqueBonusSoldier));
        }
    }
    ...
    private void resourceReducter(ActionEvent event){
        ...
        int amountSoldiersCheck = amountOfSoldiers;
        double multiplier = 1;
        ...
        for(NormalBuilding building : normalBuildingList){
            if(building.getName().getValue().equals(buttonText)){
                ...
                amountSoldiersCheck -= (building.getInitialSoldier() * multiplier * difficultyMultiplier);
                
                if(amountGoldCheck < 0 || amountWoodCheck < 0 || amountStoneCheck < 0 || amountFoodCheck < 0 || amountHumanCheck < 0 ||
                        amountIronCheck < 0 || amountCoalCheck < 0 || amountSteelCheck < 0 || amountSoldiersCheck < 0){
                ...
                if(resourceAmountCheck == false){
                    ...
                    amountOfSoldiers -= (building.getInitialSoldier() * multiplier * difficultyMultiplier);
